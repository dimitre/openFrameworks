...
#include <fmt/chrono.h> // commented out ofGen timestamp in Project.xcconfig
...
void ofTemplateMacos::addFramework(const fs::path & path) {
	...
	alert("	▸ appleFramework " + path.string(), 95);
		...
		if (folders.size()) {
			...
			for (std::size_t a = 0; a < folders.size(); a++) {
				...
				if (folderUUID.find(fullPath) != folderUUID.end()) {
					...
					std::string thisUUID { generateUUID(fullPath.string()) };
					folderUUID[fullPath] = thisUUID;
					folderFromUUID[thisUUID] = fullPath;

					std::string folderName { ofPathToString(folders[a]) };
...
void ofTemplateMacos::save() {
	...
	copyTemplateFiles[0].appends.emplace_back("// Generated by " + getVersion());
	copyTemplateFiles[0].appends.emplace_back("// " + fmt::format("{:%Y/%m/%d-%H:%M}", std::chrono::system_clock::now()));
