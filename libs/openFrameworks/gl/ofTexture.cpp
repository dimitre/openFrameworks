...
void ofTexture::setRGToRGBASwizzles(bool rToRGBSwizzles) {
	...
	if (rToRGBSwizzles) {
		...
		} else if (texData.glInternalFormat == GL_RG8 || texData.glInternalFormat == GL_RG16 || texData.glInternalFormat == GL_RG32F) {
			glTexParameteri(texData.textureTarget, GL_TEXTURE_SWIZZLE_R, GL_RED);
			glTexParameteri(texData.textureTarget, GL_TEXTURE_SWIZZLE_G, GL_RED);
			glTexParameteri(texData.textureTarget, GL_TEXTURE_SWIZZLE_B, GL_RED);
		...
		if (texData.glInternalFormat == GL_R8 || texData.glInternalFormat == GL_R16 || texData.glInternalFormat == GL_R32F) {
			glTexParameteri(texData.textureTarget, GL_TEXTURE_SWIZZLE_R, GL_RED);
...
ofMesh ofTexture::getQuad(const glm::vec3 & p1, const glm::vec3 & p2, const glm::vec3 & p3, const glm::vec3 & p4) const {
	...
	//	ofMesh quad;
	return { OF_PRIMITIVE_TRIANGLE_FAN, { { p1.x, p1.y, p1.z }, { p2.x, p2.y, p2.z }, { p3.x, p3.y, p3.z }, { p4.x, p4.y, p4.z } },
		{ { tx0, ty0 },
			{ tx1, ty0 },
			{ tx1, ty1 },
			{ tx0, ty1 } } };
