...
struct ofCubeMapSettings {
	fs::path filePath { "" }; ///< path to load the cube map, a single texture
	fs::path cacheDirectory { "" }; ///< directory to cache the irradiance and pre filter maps
...
public:
	...
	bool load(const fs::path & apath, int aFaceResolution, bool aBFlipY = true);
	/// \load An image and convert to cube map.
	/// \param apath path to the image to load.
	/// \param aFaceResolution resolution of the cube map image sides.
	/// \param aBFlipY flip the images upside down.
	...
	bool load(const fs::path & apath, int aFaceResolution, bool aBFlipY, int aIrradianceRes, int aPreFilterRes);
...
protected:
	...
	void _createIrradianceMap(GLuint aSrcCubeFid, bool aBMakeCache, const fs::path & aCachePath);
	bool _loadIrradianceMap(const fs::path & aCachePath);

	void _createPrefilteredCubeMap(GLuint aSrcCubeFid, int aSrcRes, bool aBMakeCache, const fs::path & aCachePath);
	bool _loadPrefilterMap(const fs::path & aCachePath);
