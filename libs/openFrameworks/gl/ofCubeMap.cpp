...
//----------------------------------------
bool ofCubeMap::load( const fs::path & apath, int aFaceResolution, bool aBFlipY ) {
	return load(apath, aFaceResolution, aBFlipY, 32, 128 );
}

//----------------------------------------
bool ofCubeMap::load( const fs::path & apath, int aFaceResolution, bool aBFlipY, int aIrradianceRes, int aPreFilterRes ) {
...
//	}
	...
	if( !bLoadOk ) {
		...
		if( isHdr() ) {
			...
			fs::path baseName = data->settings.filePath.stem(); // equivalent to getBaseName
			fs::path cacheIrrName { baseName };
			cacheIrrName += ("_irr_"+ofToString(data->settings.irradianceRes,0)+".exr");
			fs::path cachePrefilterName { baseName };
...
//--------------------------------------------------------------
void ofCubeMap::_createIrradianceMap(GLuint aSrcCubeFid, bool aBMakeCache, const fs::path & aCachePath) {
	if(data->bIrradianceAllocated) {
		return;
	}
	if( !data->bIrradianceAllocated ) {
...
//--------------------------------------------------------------
bool ofCubeMap::_loadIrradianceMap(const fs::path & aCachePath) {
...
//--------------------------------------------------------------
void ofCubeMap::_createPrefilteredCubeMap(GLuint aSrcCubeFid, int aSrcRes, bool aBMakeCache, const fs::path & aCachePath) {
	if(data->bPreFilteredMapAllocated) {
		return;
	}
...
//--------------------------------------------------------------
bool ofCubeMap::_loadPrefilterMap( const fs::path & aCachePath ) {
	if(data->bPreFilteredMapAllocated) {
		return false;
	}
