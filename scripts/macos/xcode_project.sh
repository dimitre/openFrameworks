...
copy_resources() {
    ...
    if [[ $ICON_FILE != "" ]] ; then
        ...
        cp $ICON_FILE "$TARGET_BUILD_DIR/$PRODUCT_NAME.app/Contents/Resources/"
        # echo rsync -aved --delete --exclude='.DS_Store' "$ICON_FILE" "$TARGET_BUILD_DIR/$PRODUCT_NAME.app/Contents/Resources/" --dry-run
    fi
}
